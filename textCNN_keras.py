...
parser = argparse.ArgumentParser(description="textCNN configuration ...")

# False: train, True: not train
parser.add_argument("--static_mode", choices=['True', 'False'], default='True',
...
parser.add_argument("--multi_channel", choices=[True, False], default=False,
                    help="use multi_channel?")

args = parser.parse_args()

if args.static_mode == 'True':
    static_mode = True
else:
    static_mode = False
...
if multi_channel:
    config_desc = "conv-1layer_multi-channel{}_train{}_embedding-{}_dim{}_filters{}_kernel-size{}_strides{}_padding{}_hidden_dim{}_drop-rate{}_batch-size{}".format(
    ...
    config_desc = "conv-1layer_train{}_embedding-{}_dim{}_filters{}_kernel-size{}_strides{}_padding{}_hidden_dim{}_drop-rate{}_batch-size{}".format(
...
class textCNN(object):
    ...
    def train(self, X_train, y_train, checkpoint_path, tensorboard_dir):
        ...
        if multi_channel:
            self.track = self.model.fit([X_train, X_train], y_train, BATCH_SIZE, EPOCH_NUM, validation_split=.1,
                                        callbacks=calllback_list, shuffle=True)
        else:
            self.track = self.model.fit(X_train, y_train, BATCH_SIZE, EPOCH_NUM, validation_split=.1,
                                        callbacks=calllback_list, shuffle=True)
...
def main():
    ...
    if os.path.exists(checkpoint_path):
        ...
        X_test, y_test = load_testset(test_datafile)
        X_test_pad = dataObj.process_and_pad_data(X_test, dataObj.tokenizer)
        y_test_1hot = dataObj.label2onehot(y_test)
        # evaluate
...
def unittest():
    print("evaluate on pseudo test ..")
    print('=' * 100)
    eval_on_test_set(os.path.join(data_dir, "pesudoTest.csv"))

    print("evaluate on 1109 ..")
    print('=' * 100)
    eval_on_test_set(os.path.join(data_dir, "test.csv"))
    # ====================
    # test 2: testSet_20181113_cut.csv
    # ===================
    print("evaluate on 1113 ..")
    print('=' * 100)
    eval_on_test_set(os.path.join(data_dir, "testSet_20181113.csv"))

    # ====================
    # test 3: testSet_20181114_cut.csv
    # ===================
    print("evaluate on 1114 ..")
    print('=' * 100)
    eval_on_test_set(os.path.join(data_dir, "testSet_20181114.csv"))
...
if __name__ == '__main__':
    ...
    # unittest()
    # eval_on_test_set(os.path.join(data_dir, "testSet_20181111.csv"))
